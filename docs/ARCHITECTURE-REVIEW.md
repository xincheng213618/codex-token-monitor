...
第二十八轮（2026-09-19，功能）：WorkBuddy 来源实际模型计价，对齐 Codex/ZCode 已有体验。WorkBuddy 会话日志（`~/.workbuddy/projects` 的 JSONL）在带 `message.usage` 的行上同根携带 `providerData.model`（如 `hy3`、`deepseek-v4-pro` 及腾讯云端点 ID `ep-…`），读取器提取它进 `TokenUsageEvent.ModelId`，桶构建统一改用事件版 `Add` 保留 `ModelUsage`；主窗口 `SupportsModelCost` 加入 WorkBuddy，输出"实际模型 · 标准 API 等价"卡（WorkBuddy 价格组）、明细表"实际模型/模型费用"两列与"换用"前缀、复制摘要行。一次性 `workbuddy-model-context-v1` 缓存维护迁移把无模型的 workbuddy 事件所在日标记重扫（只标记、绝不删除事件行，scanned_through 一并重置）。价格映射：`hy3` 直接命中价格库既有"腾讯混元 hy3"档（¥1.00/0.25/4.00 每百万）；端点 ID（`ep-…`）与 `hy4-preview`、`tencent/hy3-preview-…` 等无公开单价的模型保持 0x 待填，悬停明细逐模型列出条数与 Token，可在价格设置按模型 ID 补价。测试：`WorkBuddyUsageReaderTests` 3 项（模型提取、汇总归因、无模型行保持未识别）、`UsageCacheStoreMigrationTests` 补 workbuddy 迁移（标记重扫且不删行）、`CodexModelCostGroupTests` 补 WorkBuddy 组（hy3 人民币计价、端点 ID 不误匹配）。验证：Core 回归 656/656，桌面回归三组通过（报告 `artifacts/desktop-probes/20260919-153009-*/`）；实测（隔离缓存 + 真实日志）：9/3 的 52 条事件全部归属 hy3，实际模型费用 ¥1.08（截图时代按 K3/GLM 等估算为 $1.689/¥8.593，仅供对比），迁移标记的 10 个历史日重扫后清零。

第二十五轮补充二（同日）：ZCode 明细表此前不带模型列（列开关 includeQuota 同时控制模型列与额度列）。拆分为 includeModels + includeQuota：ZCode 显示实际模型/模型费用列与"换用"前缀，额度列仍为 Codex 专属。同日事故恢复的 177 条无模型事件按当天会话日志证据补记 GLM-5.3-Flash（当日全部 usage 记录均为该模型，无 GLM-5.2 用量记录）。验证：Core 回归 640/640，桌面回归三组通过（报告 `artifacts/desktop-probes/20260919-033723-*/`）。

第二十五轮补充（同日）：首版发布后实测发现存量用户看不到新卡——价格目录版本化迁移只重排不注入新档，且旧缓存里无模型的"今天"事件会一直压住实际模型卡。修复：`DisplayOrderVersion` 18→19，`Normalize` 在版本刷新时用 `MergeMissingDefaults` 把目录新增档合入存量各价格组（用户删除的档位仅在版本刷新时恢复）；缓存迁移曾升级为"删除无模型 zcode 事件 + 标记整日重扫 + 重置 scanned_through"——实测证明这是错误设计：ZCode CLI 会轮转 rollout 日志，当天 00:00–02:35 的记录在日志端已不存在，删除导致约 19M tokens / 177 条事件不可从日志恢复。已回退为**只标记重扫、绝不删除事件行**（重扫合并就地富化同键行，无日志的行保留计数、仅缺模型），并在测试中固化"迁移不得删除事件行"契约；夹具补齐 `scanned_through_local`/`last_token_event_local` 基础列（缺列会触发无关的加列重扫标记）。丢失数据的事后补救：对事发前保全的缓存副本做 SQLite 页面残影取证（记录头 + 串行类型逐字段解析，用现存 82 行交叉验证解析器），找回全部 177 条并写回真实缓存（model_id 为空——该窗口单条模型无法确证，计数恢复、定价缺失）；副本与脚本留存于 `work/zcode-cache-recovery-20260919/`。验证：Core 回归 640/640，桌面回归三组通过（报告 `artifacts/desktop-probes/20260919-030931-*/`）。
...
第十八轮（2026-09-19）继续补薄弱模块测试：`CodexSubscriptionPricingTests`（Sol 固定折算基准、日期后缀剥离、其他模型透传 API 档）、`CodexDataSharingSettingsTests`（默认值、全字段往返、坏 JSON 回退、越界钳制）、`QuotaCostCurveCalculatorTests`（以种子化用量缓存与 `PutTimelineSnapshots` 额度锚点驱动真实 Build：历史周期曲线点单调、缺失额度锚点不产曲线、10% 费用分带齐全）。验证：Core 回归 595/595，桌面回归三组通过（报告 `artifacts/desktop-probes/20260919-014408-*/`）。
