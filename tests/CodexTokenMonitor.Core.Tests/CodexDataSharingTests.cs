...
public sealed class CodexDataSharingTests
{
    ...
    public async Task TwoWaySharing_UsesRealHttpAndCaches_DeduplicatesRoundTrips()
    {
        ...
        source.AddEvent("source", Now, 100);
        host.AddEvent("host", Now.AddMinutes(1), 200);
        host.AddEvent("last-week", Now.AddDays(-7), 500);
    ...
    [Fact]
    public async Task UploadOutsideThisWeek_IsRejectedBeforeAnyCacheWrite()
    {
        using var source = new TestCache();
        using var host = new TestCache();
        source.AddEvent("current", Now, 100);
        source.AddEvent("previous", Now.AddDays(-7), 100);
        using var package = new CodexSharingTemporaryFile();
    ...
    private sealed class TestCache : IDisposable
    {
        ...
        public void AddEvent(string key, DateTimeOffset timestamp, long input)
        {
            UsageCacheStore.Load(Folder).MergeImportedDetailEvents(new[]
            {
                new TokenUsageEvent(timestamp, input, 20, 10, 0, input + 10, "codex:" + key)
            });
        }
        ...
        public void AddQuota(DateTimeOffset timestamp, decimal percent) =>
            ...
            {
                new CodexQuotaSnapshot(timestamp, "codex", null, null, null, percent, Now.AddDays(3))
            });

        public Task<CodexDataExportResult> ExportAsync(string path, CancellationToken cancellationToken)
        {
            var range = CodexDataTransferService.GetExportRange(CodexDataExportScope.ThisWeek, Now);
        ...
        public Task<CodexDataImportResult> ImportAsync(string path, CancellationToken cancellationToken) =>
            Task.FromResult(CodexDataTransferService.ImportThisWeek(path, cancellationToken, Folder, Now));
